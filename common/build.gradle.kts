  ...
  sourceSets {
    ...
    val androidMain by getting {
      ...
      dependencies {
        ...
        implementation("com.yubico.yubikit:piv:2.4.0")
        implementation("com.yubico.yubikit:core:2.4.0")

        // Secure Storage (EncryptedSharedPreferences)
        implementation("androidx.security:security-crypto:1.1.0-alpha06")

        // Web3j for Ethereum/EVM wallet functionality
        implementation("org.web3j:core:4.9.8")
        implementation("org.web3j:crypto:4.9.8")

        // Bitcoin BIP39/BIP32 mnemonic generation
...
afterEvaluate {
  ...
  tasks.create("adjustFormatting") {
    doLast {
      ...
      val possibleFormat = listOf("s", "d", "1\$s", "2\$s", "3\$s", "4\$s", "5\$s", "1\$d", "2\$d", "3\$d", "4\$d", "5\$d", "2s", "f")
      ...
      fun String.formatting(filepath: String): List<String> {
        ...
        val value = substringAfter("\">").substringBeforeLast("</string>").replace("%%", "")
