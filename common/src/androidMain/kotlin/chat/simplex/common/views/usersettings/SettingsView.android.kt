...
actual fun SettingsSectionApp(
  ...
  DexSettingsItem(
    icon = painterResource(MR.images.ic_restart_alt),
    text = stringResource(MR.strings.settings_restart_app),
    onClick = ::restartApp
  )
  DexSettingsItem(
    icon = painterResource(MR.images.ic_power_settings_new),
    text = stringResource(MR.strings.settings_shutdown),
    onClick = { shutdownAppAlert(::shutdownApp) }
  )
}
