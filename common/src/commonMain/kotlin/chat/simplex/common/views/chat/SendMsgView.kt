...
/** Shared state so ComposeView can read which effect was chosen */
val selectedVoiceEffect = mutableStateOf(VoiceEffect.NORMAL)
...
fun SendMsgView(
  ...
  requestKeyboard: MutableState<Boolean> = mutableStateOf(false)
  ) {
...
private fun RecordVoiceView(recState: MutableState<RecordingState>, stopRecOnNextClick: MutableState<Boolean>) {
  ...
  val showVoiceModePicker = remember { mutableStateOf(false) }

  val startRecordingWithEffect: (VoiceEffect) -> Unit = { effect ->
  ...
  if (stopRecOnNextClick.value) {
    ...
    StopRecordButton(
      onClick = stopRecordingAndAddAudio,
      isScrambled = selectedVoiceEffect.value.isScrambled
    )
  } else {
    Box {
      val interactionSource = interactionSourceWithTapDetection(
        ...
        onClick = {
          if (stopRecOnNextClick.value) {
            ...
            showVoiceModePicker.value = true
          }
        },
        onCancel = {
          if (recState.value is RecordingState.Started) stopRecordingAndAddAudio()
        },
        onRelease = {
          if (recState.value is RecordingState.Started) stopRecordingAndAddAudio()
        }
      ...
      val isFreeTier = !chat.simplex.common.views.chatlist.PremiumGate.isActive()

      // Helper — runs the action immediately if premium, otherwise dismisses the menu
      // and opens the global upgrade dialog. Voice scramble (all non-Normal effects)
      // is a Pro feature.
      val launchEffect: (VoiceEffect) -> Unit = { effect ->
        showVoiceModePicker.value = false
        if (effect == VoiceEffect.NORMAL) {
          stopRecOnNextClick.value = true
          startRecordingWithEffect(effect)
        } else {
      ...
      DropdownMenu(
        expanded = showVoiceModePicker.value,
        onDismissRequest = { showVoiceModePicker.value = false },
        modifier = Modifier.background(MaterialTheme.colors.surface)
      ) {
        // Normal voice (free)
        VoiceEffectMenuItem(VoiceEffect.NORMAL, Color(0xFF1F4CFF), MR.images.ic_keyboard_voice, locked = false) {
          launchEffect(VoiceEffect.NORMAL)
        }
        // Paranoid — irreversible (Pro)
        VoiceEffectMenuItem(VoiceEffect.PARANOID, Color(0xFFD20D0D), MR.images.ic_shield, locked = isFreeTier) {
          launchEffect(VoiceEffect.PARANOID)
        }
        // Random (Pro)
        VoiceEffectMenuItem(VoiceEffect.RANDOM, Color(0xFF9C27B0), MR.images.ic_bolt, locked = isFreeTier) {
          launchEffect(VoiceEffect.RANDOM)
        }
        // 6 distinct effects (Pro)
        VoiceEffectMenuItem(VoiceEffect.VILLAIN, Color(0xFFFF4444), MR.images.ic_bolt, locked = isFreeTier) {
          launchEffect(VoiceEffect.VILLAIN)
        }
        VoiceEffectMenuItem(VoiceEffect.HELIUM, Color(0xFFFFEB3B), MR.images.ic_bolt, locked = isFreeTier) {
          launchEffect(VoiceEffect.HELIUM)
        }
        VoiceEffectMenuItem(VoiceEffect.ROBOT, Color(0xFF607D8B), MR.images.ic_bolt, locked = isFreeTier) {
          launchEffect(VoiceEffect.ROBOT)
        }
        VoiceEffectMenuItem(VoiceEffect.DEMON, Color(0xFF9C27B0), MR.images.ic_bolt, locked = isFreeTier) {
          launchEffect(VoiceEffect.DEMON)
        }
        VoiceEffectMenuItem(VoiceEffect.GHOST, Color(0xFF8BC34A), MR.images.ic_bolt, locked = isFreeTier) {
          launchEffect(VoiceEffect.GHOST)
        }
        VoiceEffectMenuItem(VoiceEffect.CYBORG, Color(0xFF00BCD4), MR.images.ic_bolt, locked = isFreeTier) {
          launchEffect(VoiceEffect.CYBORG)
        }
      }
    }
  }
...
@Composable
private fun VoiceEffectMenuItem(
  effect: VoiceEffect,
  color: Color,
  iconRes: dev.icerock.moko.resources.ImageResource,
  ...
  DropdownMenuItem(onClick = onClick) {
    Row(
      ...
      Icon(
        ...
        tint = if (locked) color.copy(alpha = 0.55f) else color,
      ...
      Text(
        ...
        color = when {
          effect == VoiceEffect.NORMAL -> MaterialTheme.colors.onSurface
          locked -> color.copy(alpha = 0.6f)
          else -> color
        },
      ...
      if (locked) {
        ...
        Box(
          modifier = Modifier
            ...
            .background(Color(0xFF1F4CFF).copy(alpha = 0.15f))
...
@Composable
private fun StopRecordButton(onClick: () -> Unit, isScrambled: Boolean = false) {
  val interactionSource = remember { MutableInteractionSource() }
  val ripple = remember { ripple(bounded = false, radius = 24.dp) }
  val bgColor = if (isScrambled) Color(0xFFFF4444) else Color(0xFF1F4CFF)
  ...
  Box(
    ...
    Box(
      modifier = Modifier
        ...
        .alpha(alpha.value)
        .clip(CircleShape)
        .background(if (enabled) Color(0xFF1F4CFF) else MaterialTheme.colors.secondary.copy(alpha = 0.75f)),
      contentAlignment = Alignment.Center
    ) {
      Icon(
        icon,
