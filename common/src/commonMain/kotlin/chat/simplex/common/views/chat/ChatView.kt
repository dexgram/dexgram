...
fun ChatView(
  ...
  if (chat == null || chatInfo == null || user == null) {
    ...
    if (appPlatform.isAndroid) {
      ...
      // Track keyboard height for smooth emoji picker transitions
      // Also auto-closes emoji picker when keyboard becomes visible
      KeyboardHeightTracker(showEmojiPicker)
    }
    LaunchedEffect(Unit) {
      ...
      is ChatInfo.Direct, is ChatInfo.Group, is ChatInfo.Local -> {
        ...
        SimpleXThemeOverride(overrides ?: CurrentColors.collectAsState().value) {
          val onSearchValueChanged: (String) -> Unit = onSearchValueChanged@{ value ->
            ...
            withBGApi {
              apiFindMessages(chatsCtx, c, search = value)
          ...
          ChatLayout(
            ...
            loadMessages = { chatId, pagination, visibleItemIndexes ->
              ...
              if (c != null) {
                apiLoadMessages(chatsCtx, c.remoteHostId, c.chatInfo.chatType, c.chatInfo.apiId, pagination, search = searchText.value, visibleItemIndexesNonReversed = visibleItemIndexes)
              }
            },
      ...
      Box(Modifier.fillMaxSize().background(MaterialTheme.colors.background)) {
        ...
        AdaptingBottomPaddingLayout(Modifier, CHAT_COMPOSE_LAYOUT_ID, composeViewHeight) {
          ...
          if (chatsCtx.contentTag == MsgContentTag.Report) {
            ...
            // Unified compose container with keyboard/emoji picker management
            // Signal-like behavior: chat content stays stable, only bottom panel transitions
            Column(
              Modifier
                .layoutId(CHAT_COMPOSE_LAYOUT_ID)
                .align(Alignment.BottomCenter)
                .navigationBarsPadding()
                .imePadding() // Always apply imePadding for keyboard
            ) {
              composeView(composeViewFocusRequester)
              // Emoji picker overlays keyboard area when visible
              if (chat != null) {
                EmojiPickerSheet(showEmojiPicker) { emoji ->
                  val currentText = composeState.value.message.text
                  val newText = currentText + emoji
                  composeState.value = composeState.value.copy(message = ComposeMessage(newText))
                }
              }
            }
          }
    ...
    barButtons.add {
      IconButton(
        ...
        Icon(
          ...
          tint = if (chatInfo.noteFolder.ready) MaterialTheme.colors.onBackground else MaterialTheme.colors.secondary
        )
      }
    }
  } else {
      ...
      barButtons.add {
        IconButton({
          ...
          Icon(
            ...
            tint = if (chatInfo.contact.ready && chatInfo.contact.active) MaterialTheme.colors.onBackground else MaterialTheme.colors.secondary
          )
        }
      }
      // Voice call button second
      barButtons.add {
        IconButton({
          showMenu.value = false
    ...
    if (!chatInfo.incognito) {
      barButtons.add {
        IconButton({
          ...
          Icon(painterResource(MR.images.ic_person_add_500), stringResource(MR.strings.icon_descr_add_members), tint = MaterialTheme.colors.onBackground)
        }
      }
    } else {
      barButtons.add {
        IconButton({
          ...
          Icon(painterResource(MR.images.ic_add_link), stringResource(MR.strings.group_link), tint = MaterialTheme.colors.onBackground)
        }
      }
    }
  }
  ...
  if (menuItems.isNotEmpty()) {
    barButtons.add {
      IconButton({ showMenu.value = true }) {
        Icon(MoreVertFilled, stringResource(MR.strings.icon_descr_more_button), tint = MaterialTheme.colors.onBackground)
      }
    }
  }
  DefaultAppBar(
    navigationButton = { if (appPlatform.isAndroid || showSearch.value) { NavigationButtonBack(onBackClicked, tintColor = MaterialTheme.colors.onBackground) }  },
...
fun ChatInfoToolbarTitle(cInfo: ChatInfo, imageSize: Dp = 40.dp, iconColor: Color = MaterialTheme.colors.secondaryVariant.mixWith(MaterialTheme.colors.onBackground, 0.97f)) {
  Row(
    ...
    Column(
      ...
      // Show disappearing message time under the chat name
      val disappearingTTL = cInfo.timedMessagesTTL
      if (disappearingTTL != null && (cInfo is ChatInfo.Direct || cInfo is ChatInfo.Group)) {
        Row(
          verticalAlignment = Alignment.CenterVertically,
        ) {
          Icon(
            painterResource(MR.images.ic_timer),
            contentDescription = stringResource(MR.strings.delete_after),
            modifier = Modifier.size(12.dp),
            tint = MaterialTheme.colors.secondary
          )
          Spacer(Modifier.width(3.dp))
          Text(
            shortTimeText(disappearingTTL),
            maxLines = 1,
  ...
  Box {
    ...
    Row(
      ...
      if (
        ...
        val reportsBackground = if (isDark) 
        ...
        Row(
          ...
          Icon(painterResource(MR.images.ic_flag), null, Modifier.size(20.dp), tint = MaterialTheme.colors.error)
      ...
      if (supportUnreadCount > 0) {
        val supportBackground = if (isDark) 
        ...
        Row(
          ...
          Icon(painterResource(MR.images.ic_flag), null, Modifier.size(20.dp), tint = MaterialTheme.colors.primary)
          Spacer(Modifier.width(6.dp))
          Text(
...
fun BoxScope.ChatItemsList(
  ...
  @Composable
  fun ChatBannerView() {
    // Theme-aware encryption info box
    val isDark = isInDarkTheme()
    val boxBackground = if (isDark) MaterialTheme.colors.surface else Color(0xFFF5F7FA)
    val iconColor = MaterialTheme.colors.primary.copy(alpha = 0.8f)
    val textColor = MaterialTheme.colors.onSurface.copy(alpha = 0.7f)
    val borderColor = if (isDark) MaterialTheme.colors.primary.copy(alpha = 0.2f) else Color(0xFFE0E5EC)
    ...
    Box(
      ...
      Row(
        ...
        Text(
          text = stringResource(MR.strings.e2ee_info_pq),
  ...
  LazyColumnWithScrollBar(
    ...
    itemsIndexed(mergedItemsValue.items, key = { _, merged -> keyForItem(merged.newest().item) }) { index, merged ->
      ...
      if (item.content is CIContent.ChatBanner) {
        Column {
          // Encryption info box only - no contact banner
          ChatBannerView()
...
private fun BoxScope.BottomEndFloatingButton(
  ...
  val isVisible = (showButtonWithCounter.value || showButtonWithArrow.value) && !animatedScrollingInProgress.value
  val hasUnread = showButtonWithCounter.value
  ...
  AnimatedVisibility(
    ...
    FloatingActionButton(
      ...
      backgroundColor = if (hasUnread) MaterialTheme.colors.primary else MaterialTheme.colors.surface,
      contentColor = if (hasUnread) MaterialTheme.colors.onPrimary else MaterialTheme.colors.primary
    ) {
...
fun SelectedListItem(
  ...
  val scale by animateFloatAsState(
    targetValue = if (checked.value) 1f else 0.9f,
    animationSpec = spring(dampingRatio = 0.6f, stiffness = 500f)
  )
  val uncheckedColor = if (isInDarkTheme()) {
  ...
  Box(
    modifier = modifier
      .size(24.dp * fontSizeMultiplier)
      .scale(scale)
      .background(
        color = if (checked.value) MaterialTheme.colors.primary else Color.Transparent,
        shape = CircleShape
      )
      .then(
        if (!checked.value) Modifier.border(
          width = 2.dp,
    ...
    if (checked.value) {
      Icon(
        painterResource(MR.images.ic_check),
        null,
        modifier = Modifier.size(16.dp * fontSizeMultiplier),
...
fun PreviewChatLayout() {
  SimpleXTheme {
    ...
    ChatLayout(
      ...
      showEmojiPicker = remember { mutableStateOf(false) },
      composeView = { _ -> },
