...
fun ContactPreferencesView(
  ...
  val originalFeatures = contactUserPrefsToFeaturesAllowed(ct.mergedPreferences)
  // Default timed messages to ON with 1 week if currently OFF
  val initialFeatures = originalFeatures.let { features ->
    if (!features.timedMessagesAllowed) {
      features.copy(timedMessagesAllowed = true, timedMessagesTTL = 7 * 86400)
    } else {
      features
    }
  }
  var featuresAllowed by rememberSaveable(ct, user, stateSaver = serializableSaver()) { mutableStateOf(initialFeatures) }
  var currentFeaturesAllowed by rememberSaveable(ct, user, stateSaver = serializableSaver()) { mutableStateOf(originalFeatures) }
  ...
  // Auto-save timed messages with 1 week if it was OFF (one-time auto-apply)
  LaunchedEffect(ct.contactId) {
    if (!originalFeatures.timedMessagesAllowed) {
      savePrefs()
    }
  }
...
private fun TimedMessagesFeatureSection(
  ...
  SectionView(
    ...
    PreferenceToggle(
      ...
      val ttlWhenEnabling =
        if (!featuresAllowed.timedMessagesAllowed && (featuresAllowed.timedMessagesTTL == null || featuresAllowed.timedMessagesTTL == oldDefaultMonth)) {
          week
        } else {
          featuresAllowed.timedMessagesTTL ?: week
        }
      onSelected(allow, if (allow) ttlWhenEnabling else null)
    }
    InfoRow(
      generalGetString(MR.strings.chat_preferences_contact_allows),
      pref.contactPreference.allow.text
