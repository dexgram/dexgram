...
fun MarkdownText (
  ...
  // Use LocalContentColor from Surface, or fall back to onSurface if no style is provided
  val effectiveStyle = style ?: MaterialTheme.typography.body1.copy(
  ...
  CompositionLocalProvider(
    ...
    if (formattedText == null) {
      ...
      Text(annotatedText, style = effectiveStyle, modifier = modifier, maxLines = maxLines, overflow = overflow, inlineContent = inlineContent?.second ?: mapOf())
    } else {
      var hasLinks = false
      ...
      if ((hasLinks && uriHandler != null) || hasSecrets || (hasCommands && sendCommandMsg != null)) {
        ...
        ClickableText(annotatedText, style = effectiveStyle, modifier = modifier.pointerHoverIcon(icon.value), maxLines = maxLines, overflow = overflow,
          onLongClick = { offset ->
            if (hasLinks) {
...
fun openBrowserAlert(uri: String, uriHandler: UriHandler) {
  // Check if this is a shredgram encrypted link - handle it directly without opening browser
  if (ShredgramLinkEncoder.isShredgramLink(uri)) {
    val decryptedLink = ShredgramLinkEncoder.decode(uri)
    if (decryptedLink != null) {
      // Process the decrypted link directly within the app
      withBGApi {
        planAndConnect(
