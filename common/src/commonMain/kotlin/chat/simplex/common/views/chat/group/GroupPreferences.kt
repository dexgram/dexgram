...
fun GroupPreferencesView(m: ChatModel, rhId: Long?, chatId: String, close: () -> Unit) {
  ...
  val originalPreferences = gInfo.fullGroupPreferences
  // Default timed messages to ON with 1 week if currently OFF (only if owner)
  val initialPreferences = originalPreferences.let { prefs ->
    if (gInfo.isOwner && prefs.timedMessages.enable == GroupFeatureEnabled.OFF) {
      prefs.copy(timedMessages = TimedMessagesGroupPreference(enable = GroupFeatureEnabled.ON, ttl = 7 * 86400))
    } else {
      prefs
    }
  }
  var preferences by rememberSaveable(gInfo, stateSaver = serializableSaver()) { mutableStateOf(initialPreferences) }
  var currentPreferences by rememberSaveable(gInfo, stateSaver = serializableSaver()) { mutableStateOf(originalPreferences) }
  ...
  // Auto-save timed messages with 1 week if it was OFF (one-time auto-apply, only if owner)
  LaunchedEffect(gInfo.groupId) {
    if (gInfo.isOwner && originalPreferences.timedMessages.enable == GroupFeatureEnabled.OFF) {
      savePrefs()
    }
  }
  ...
  ColumnWithScrollBar {
    ...
    FeatureSection(GroupFeature.TimedMessages, timedMessages, null, groupInfo, preferences, onTTLUpdated) { enable, _ ->
      if (enable == GroupFeatureEnabled.ON) {
        val week = 7 * 86400
        val oldDefaultMonth = 30 * 86400
        val ttlWhenEnabling =
          if (preferences.timedMessages.enable == GroupFeatureEnabled.OFF && (preferences.timedMessages.ttl == null || preferences.timedMessages.ttl == oldDefaultMonth)) {
            week
          } else {
            preferences.timedMessages.ttl ?: week
          }
        applyPrefs(preferences.copy(timedMessages = TimedMessagesGroupPreference(enable = enable, ttl = ttlWhenEnabling)))
      } else {
