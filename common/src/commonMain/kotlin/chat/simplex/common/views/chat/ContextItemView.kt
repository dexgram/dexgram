...
fun ContextItemView(
  ...
  val sentColor = Color(0xFFE7EBF2) // Light purple-gray for sent messages
  val receivedColor = Color.White // White for received messages
  ...
  fun MessageText(contextItem: ChatItem, attachment: ImageResource?, lines: Int) {
    ...
    val isSpecialMsg = PaymentInvoice.isPaymentMessage(rawText) || BotMessage.isBotProtocol(rawText)
    val displayText = when {
      PaymentInvoice.isPaymentMessage(rawText) -> PaymentInvoice.humanReadableText(rawText)
      BotMessage.isBotProtocol(rawText) -> BotMessage.humanReadableText(rawText)
      else -> rawText
    }
    MarkdownText(
      displayText, if (isSpecialMsg) null else contextItem.formattedText,
      sender = null,
      toggleSecrets = false,
      maxLines = lines,
