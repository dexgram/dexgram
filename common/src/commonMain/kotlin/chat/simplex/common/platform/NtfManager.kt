...
abstract class NtfManager {
  ...
  private suspend fun awaitChatStartedIfNeeded(chatModel: ChatModel, timeout: Long = 30_000) {
    ...
    if (chatModel.chatRunning.value == null) {
      ...
      for (i in 0..(timeout / step)) {
        val stage = chatModel.controller.appPrefs.onboardingStage.get()
        if (chatModel.chatRunning.value == true || stage == OnboardingStage.Step1_SimpleXInfo || stage == OnboardingStage.Step0_1_FeatureNoIdentity) {
          break
        }
        delay(step)
      }
  ...
  private fun hideSecrets(cItem: ChatItem): String {
    val rawText = cItem.text
    if (PaymentInvoice.isPaymentMessage(rawText)) {
      return PaymentInvoice.humanReadableText(rawText)
    }
    if (BotMessage.isBotProtocol(rawText)) {
      return BotMessage.humanReadableText(rawText)
    }
    ...
    return if (md != null) {
      ...
      if (mc is MsgContent.MCReport) {
        generalGetString(MR.strings.notification_group_report).format(rawText.ifEmpty { mc.reason.text })
      } else {
        rawText
      }
    }
