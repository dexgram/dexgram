...
object ChatModel {
  ...
  // YubiKey NFC detection states
  val yubiKeyDetected = mutableStateOf(false)
  val yubiKeyEnrollmentResult = mutableStateOf<Result<String>?>(null)
  val yubiKeyDatabaseKey = mutableStateOf<String?>(null)
  val currentYubiKeyTag = mutableStateOf<Any?>(null) // Platform-specific tag object
  @Deprecated("Use secureYubiKeyPin instead for secure memory handling")
  val currentYubiKeyPin = mutableStateOf<String?>(null)
  // SECURITY: Use SecurePin for proper memory wiping of PIN
  val secureYubiKeyPin = SecurePin()
  // SECURITY: Temporary PUK storage for unblocking YubiKey PIN after lockout
  // This is cleared after successful PIN unlock
  val secureYubiKeyPuk = SecurePin()
  ...
  val userUsername = mutableStateOf<String?>(null)
  val publicUsername = mutableStateOf<String?>(null)
  val usernameConnectionId = mutableStateOf<Long?>(null)

  private val _usernameUsed = mutableStateOf(false)
  private var _usernameUsedLoaded = false

  fun setUsernameUsed(used: Boolean) {
    _usernameUsed.value = used
    _usernameUsedLoaded = true
    try {
      ChatController.appPrefs.usernameUsedPersisted.set(used)
    } catch (e: Exception) { }
  }

  fun setUsernameExpiry() {
    try {
      val expiryTimestamp = System.currentTimeMillis() + (5 * 60 * 1000L)
      ChatController.appPrefs.usernameExpiryTimestamp.set(expiryTimestamp)
    } catch (e: Exception) { }
  }

  fun clearUsernameExpiry() {
    try {
      ChatController.appPrefs.usernameExpiryTimestamp.set(0L)
    } catch (e: Exception) { }
  }

  fun generateUsernamePrefix(): String {
    val letters = ('a'..'z').toList()
    return (1..5).map { letters.random() }.joinToString("")
  }

  fun generateLocalPublicUsername(): String {
    val letters = ('a'..'z').toList()
    val digits = ('0'..'9').toList()
    val randomLetters = (1..5).map { letters.random() }.joinToString("")
    val randomDigits = (1..3).map { digits.random() }.joinToString("")
    return "$randomLetters.$randomDigits.link"
  }

  fun getPublicUsername(): String? {
    if (publicUsername.value != null) return publicUsername.value
    return try {
      val stored = ChatController.appPrefs.publicUsername.get()
      if (stored != null) publicUsername.value = stored
      stored
    } catch (e: Exception) { null }
  }

  fun setPublicUsername(username: String) {
    publicUsername.value = username
    try {
      ChatController.appPrefs.publicUsername.set(username)
    } catch (e: Exception) { }
  }

  fun initPublicUsernameIfNeeded(): String {
    val existing = getPublicUsername()
    if (existing != null) return existing
    val newUsername = generateLocalPublicUsername()
    setPublicUsername(newUsername)
    return newUsername
  }

  fun getPrivateUsername(): String? {
    if (userUsername.value != null) return userUsername.value
    return try {
      val stored = ChatController.appPrefs.privateUsername.get()
      if (stored != null) userUsername.value = stored
      stored
    } catch (e: Exception) { null }
  }

  fun setPrivateUsername(username: String) {
    userUsername.value = username
    try {
      ChatController.appPrefs.privateUsername.set(username)
    } catch (e: Exception) { }
  }

  fun isUsernameUsed(): Boolean {
    if (!_usernameUsedLoaded) {
      try {
        _usernameUsed.value = ChatController.appPrefs.usernameUsedPersisted.get()
        _usernameUsedLoaded = true
      } catch (e: Exception) { }
    }
    return _usernameUsed.value
  }

  fun isUsernameExpired(): Boolean {
    return try {
      val expiryTimestamp = ChatController.appPrefs.usernameExpiryTimestamp.get()
      expiryTimestamp > 0 && System.currentTimeMillis() > expiryTimestamp
    } catch (e: Exception) { false }
  }

  fun getUsernameTimeRemaining(): Long {
    return try {
      val expiryTimestamp = ChatController.appPrefs.usernameExpiryTimestamp.get()
      if (expiryTimestamp <= 0) return 0L
      val remaining = expiryTimestamp - System.currentTimeMillis()
      if (remaining > 0) remaining else 0L
    } catch (e: Exception) { 0L }
  }

  fun isPublicProfileSetupCompleted(): Boolean {
    return try {
      ChatController.appPrefs.publicProfileSetupCompleted.get()
    } catch (e: Exception) { false }
  }

  fun setPublicProfileSetupCompleted(completed: Boolean) {
    try {
      ChatController.appPrefs.publicProfileSetupCompleted.set(completed)
    } catch (e: Exception) { }
  }

  val networkStatuses = mutableMapOf<String, NetworkStatus>()
...
enum class NotificationPreviewMode {
  ...
  companion object {
    val default: NotificationPreviewMode = HIDDEN
  }
}
