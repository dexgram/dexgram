...
// Shredgram font families - Manrope for headlines/titles, DMSans for body/labels
...
object ThemeManager {
  ...
  private fun systemDarkThemeColors(): Pair<Colors, DefaultTheme> = when (appPrefs.systemDarkTheme.get()) {
    ...
    DefaultTheme.DARKLINK.themeName -> DarkLinkColorPalette to DefaultTheme.DARKLINK
  ...
  fun currentColors(themeOverridesForType: WallpaperType?, perChatTheme: ThemeModeOverride?, perUserTheme: ThemeModeOverrides?, appSettingsTheme: List<ThemeOverrides>): ActiveTheme {
    ...
    val baseTheme = when (nonSystemThemeName) {
      ...
      DefaultTheme.DARKLINK.themeName -> ActiveTheme(DefaultTheme.DARKLINK.themeName, DefaultTheme.DARKLINK, DarkLinkColorPalette, DarkLinkColorPaletteApp, AppWallpaper(type = PresetWallpaper.SCHOOL.toType(DefaultTheme.DARKLINK)))
