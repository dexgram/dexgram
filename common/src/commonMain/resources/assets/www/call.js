...
let voiceScrambleEnabled = false;
let scrambleAudioContext = null;
let scrambleNodes = [];
let scrambleOriginalTrack = null;
let scrambleProcessedStream = null;
let scrambleReplacedSender = null;
let scrambleCurrentEffect = "";

const SCRAMBLE_KEYS = ["villain", "helium", "robot", "demon", "ghost", "cyborg"];

async function findAudioSender(call) {
    const transceivers = call.connection.getTransceivers();
    let sender = null;
    for (const t of transceivers) {
        if (t.mid === "0" || t.mid === 0) { sender = t.sender; break; }
    }
    if (!sender) {
        for (const t of transceivers) {
            if (t.sender && t.sender.track && t.sender.track.kind === "audio") { sender = t.sender; break; }
        }
    }
    if (!sender) {
        for (const s of call.connection.getSenders()) {
            if (s.track && s.track.kind === "audio") { sender = s; break; }
        }
    }
    return sender;
}

// Helper: create pitch-shift nodes (modulated delay)
function createPitchShift(ctx, semitones) {
    ...
    lfo.type = "sawtooth";
    lfo.frequency.value = (1 - pitchRatio) / grainSize;
    lfoGain.gain.value = grainSize * Math.abs(1 - pitchRatio);
    lfo.connect(lfoGain);
...
// Build effect-specific audio chain, returns { nodes[], lastNode }
function buildEffectChain(ctx, source, effectKey) {
    const nodes = [source];
    let last = source;
    function chain(node) { last.connect(node); last = node; nodes.push(node); }
    switch (effectKey) {
        case "villain": {
            const ps = createPitchShift(ctx, -7);
            nodes.push(ps.lfo, ps.lfoGain);
            chain(ps.delay);
            const lp = ctx.createBiquadFilter(); lp.type = "lowpass"; lp.frequency.value = 2800; lp.Q.value = 0.7;
            chain(lp);
            const bass = ctx.createBiquadFilter(); bass.type = "lowshelf"; bass.frequency.value = 300; bass.gain.value = 6;
            chain(bass);
        ...
        case "robot": {
            ...
            const trLfo = ctx.createOscillator(); trLfo.type = "sine"; trLfo.frequency.value = 30;
            const trDepth = ctx.createGain(); trDepth.gain.value = 0.6;
            trLfo.connect(trDepth); trDepth.connect(trem.gain); trLfo.start();
            nodes.push(trLfo, trDepth);
            chain(trem);
        ...
        case "demon": {
            const ps = createPitchShift(ctx, -10);
            nodes.push(ps.lfo, ps.lfoGain);
            chain(ps.delay);
            const lp = ctx.createBiquadFilter(); lp.type = "lowpass"; lp.frequency.value = 2000; lp.Q.value = 0.7;
            chain(lp);
            const bass = ctx.createBiquadFilter(); bass.type = "lowshelf"; bass.frequency.value = 300; bass.gain.value = 10;
            chain(bass);
        ...
        case "ghost": {
            const ps = createPitchShift(ctx, -3);
            nodes.push(ps.lfo, ps.lfoGain);
            chain(ps.delay);
            const dryGain = ctx.createGain(); dryGain.gain.value = 0.75;
            const echoDelay = ctx.createDelay(0.5); echoDelay.delayTime.value = 0.12;
            const wetGain = ctx.createGain(); wetGain.gain.value = 0.45;
            const merger = ctx.createGain(); merger.gain.value = 1.0;
            last.connect(dryGain); dryGain.connect(merger);
            last.connect(echoDelay); echoDelay.connect(wetGain); wetGain.connect(merger);
            last = merger;
            nodes.push(dryGain, echoDelay, wetGain, merger);
            const lp = ctx.createBiquadFilter(); lp.type = "lowpass"; lp.frequency.value = 3200; lp.Q.value = 0.7;
            chain(lp);
            const bass = ctx.createBiquadFilter(); bass.type = "lowshelf"; bass.frequency.value = 300; bass.gain.value = 3;
            chain(bass);
            const g = ctx.createGain(); g.gain.value = 1.3;
            chain(g);
            break;
        ...
        case "cyborg": {
            const ps = createPitchShift(ctx, 4);
            nodes.push(ps.lfo, ps.lfoGain);
            chain(ps.delay);
            ...
            chain(ws);
            const lp = ctx.createBiquadFilter(); lp.type = "lowpass"; lp.frequency.value = 5000; lp.Q.value = 0.7;
            chain(lp);
            const g = ctx.createGain(); g.gain.value = 1.3;
            chain(g);
            break;
    ...
    return { nodes, lastNode: last };
}

async function enableVoiceScramble(call, effectName) {
    if (!call) return;
    if (voiceScrambleEnabled) { await disableVoiceScramble(call); }
    try {
        let key = (effectName || "villain").toLowerCase();
        if (key === "random") { key = SCRAMBLE_KEYS[Math.floor(Math.random() * SCRAMBLE_KEYS.length)]; }
        scrambleCurrentEffect = key;
        const audioTracks = call.localStream.getAudioTracks();
        if (audioTracks.length === 0) { console.log("Voice scramble: no audio tracks"); return; }
        const originalTrack = audioTracks[0];
        scrambleOriginalTrack = originalTrack;
        const ctx = new AudioContext({ sampleRate: 48000 });
        scrambleAudioContext = ctx;
        if (ctx.state === "suspended") await ctx.resume();
        const sourceStream = new MediaStream([originalTrack]);
        const source = ctx.createMediaStreamSource(sourceStream);
        const dest = ctx.createMediaStreamDestination();
        const { nodes, lastNode } = buildEffectChain(ctx, source, key);
        lastNode.connect(dest);
        nodes.push(dest);
        scrambleNodes = nodes;
        scrambleProcessedStream = dest.stream;
        const processedTrack = dest.stream.getAudioTracks()[0];
        if (!processedTrack) { console.log("Voice scramble: no processed track"); return; }
        const sender = await findAudioSender(call);
        if (sender) {
            await sender.replaceTrack(processedTrack);
            scrambleReplacedSender = sender;
            voiceScrambleEnabled = true;
            console.log("Voice scramble ON: " + key);
        } else {
            console.log("Voice scramble: no audio sender found");
            ctx.close(); scrambleAudioContext = null; scrambleNodes = [];
        }
    } catch (e) {
        console.log("Error enabling voice scramble: " + e + " / " + e.stack);
        voiceScrambleEnabled = false;
    }
}

async function disableVoiceScramble(call) {
    if (!voiceScrambleEnabled && !scrambleAudioContext) return;
    try {
        if (scrambleOriginalTrack && call) {
            const sender = scrambleReplacedSender || (await findAudioSender(call));
            if (sender) { await sender.replaceTrack(scrambleOriginalTrack); }
        }
        for (const node of scrambleNodes) {
            try { if (node instanceof OscillatorNode) node.stop(); } catch (_) {}
            try { node.disconnect(); } catch (_) {}
        }
        if (scrambleAudioContext) { await scrambleAudioContext.close(); }
        scrambleAudioContext = null; scrambleNodes = []; scrambleOriginalTrack = null;
        scrambleProcessedStream = null; scrambleReplacedSender = null; scrambleCurrentEffect = "";
        voiceScrambleEnabled = false;
        console.log("Voice scramble DISABLED");
    } catch (e) {
        console.log("Error disabling voice scramble: " + e);
        voiceScrambleEnabled = false;
    }
}
...
const processCommand = (function () {
        ...
        try {
            switch (command.type) {
                ...
                case "scramble":
                    if (command.enable) {
                        if (activeCall) {
                            await enableVoiceScramble(activeCall, command.effect || "villain");
                            resp = { type: "ok" };
                        } else {
                            resp = { type: "error", message: "scramble: no active call" };
                        }
                    } else {
                        if (activeCall) {
                            await disableVoiceScramble(activeCall);
                            resp = { type: "ok" };
                        } else {
                            resp = { type: "error", message: "scramble: no active call" };
                        }
    ...
    function endCall() {
        var _a;
        // Force-clean voice scramble state on call end
        if (voiceScrambleEnabled || scrambleAudioContext) {
            try {
                for (const node of scrambleNodes) {
                    try { if (node instanceof OscillatorNode) node.stop(); } catch(_){}
                    try { node.disconnect(); } catch(_){}
                }
                if (scrambleAudioContext) scrambleAudioContext.close();
            } catch(_){}
            scrambleAudioContext = null; scrambleNodes = []; scrambleOriginalTrack = null;
            scrambleProcessedStream = null; scrambleReplacedSender = null; voiceScrambleEnabled = false;
        }
